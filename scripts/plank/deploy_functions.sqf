...
plank_deploy_fnc_addPlacementActions = {
    ...
    _confirmActionId = _unit addAction ['<t color="#3748E3">Confirm Deployment</t>', "scripts\plank\confirm_fort_action.sqf", [], 100, false, false, "", "driver _target == _this"];
    _cancelActionId = _unit addAction ['<t color="#FF0000">Cancel Deployment</t>', "scripts\plank\cancel_fort_action.sqf", [], 99, false, false, "", "driver _target == _this"];
    _openActionId = _unit addAction ['<t color="#00FF00">Open Settings</t>', "scripts\plank\open_settings_action.sqf", [], 98, false, false, "", "driver _target == _this"];
...
plank_deploy_fnc_addFortificationAction = {
    ...
    if (_count > 0 && {_unit getVariable ["plank_deploy_fortIndex", -1] != _fortIndex}) then {
        ...
        _actionId = _unit addAction [format ["Place %1 (%2 left)", GET_FORT_DISPLAY_NAME(_fortIndex), _count], "scripts\plank\place_fort_action.sqf", [_fortIndex], _fortIndex + 50, false, false, "", "driver _target == _this"];
        _fortActionIds = _unit getVariable ["plank_deploy_fortActionIds", []];
